...
class Fred(BaseAPI):
    ...
    def _set_series_params(
        ...
        if not isinstance(last_data, bool):
            raise ValueError("last_data must be a booleano.")
        ...
        if last_data:
            ...
            if start_date is not None:
                ...
                if start_date > end_date:
                    ...
                    print(
                        "start_date is greater than end_date. The dates have been switched."
                    )
    ...
    def get_series_metadata(self, serie_id: str | list) -> dict:  # Hace falta modificar
        ...
        series_dict[serie_id] = {
            "title": serie_metadata["title"],
            "frequency": serie_metadata["frequency"],
            "observation_start": pd.to_datetime(
                serie_metadata["observation_start"]
            ).date(),
            "observation_end": pd.to_datetime(serie_metadata["observation_end"]).date(),
            "units": serie_metadata["units"],
            "seasonal_adjustment": serie_metadata["seasonal_adjustment"],
            "last_updated": serie_metadata["last_updated"],
            "notes": serie_metadata["notes"],
        }
    ...
    def get_series_data(
        ...
        for id in serie_id:
            ...
            time_periods_formatted = [
                pd.to_datetime(period).date() for period in time_periods
            ]
    ...
    def get_releases_data(
        ...
        # Extraer los valores y las fechas
        obs_values = [
            float(entry["value"].replace(",", "")) if entry["value"] != "N/E" else pd.NA
            for entry in serie_data
        ]
        time_periods = [entry["date"] for entry in serie_data]
