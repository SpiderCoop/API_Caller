...
class INEGI_BIE(BaseAPI):
    ...
    def _unit_handler(self, unit_id: int):
        ...
        # Definir url de API
        endpoint = (
            f"/CL_UNIT/{unit_id}/es/BIE-BISE/2.0/{self._BaseAPI__api_key}?type=json"
        )
    ...
    def _transform_time_periods(self, time_periods: list, frequency_id: int):
        ...
        if frequency_id == 1 or frequency_id == 2 or frequency_id == 3:  # 10 años
            raise ValueError(
                "Frecuencia no soportada actualmente. Es necesario modificar el codigo para soportar esta frecuencia."
            )
            ...
            for period in time_periods:
                ...
                time_periods_formatted.append(
                    pd.Timestamp(year=year, month=quarter * 3, day=1).date()
                )

        elif frequency_id == 8:  # Mensual
            time_periods_formatted = [
                pd.to_datetime(period, dayfirst=True).strftime("%Y-%m-%d")
                for period in time_periods
            ]

        elif frequency_id == 9 or frequency_id == 10 or frequency_id == 11:  # Quincenal
            raise ValueError(
                "Frecuencia no soportada actualmente. Es necesario modificar el codigo para soportar esta frecuencia."
            )
    ...
    def _set_series_params(self, serie_id: str | list, last_data: bool = False) -> str:
        ...
        last_data = "true" if last_data else "false"
    ...
    def get_series_data(
        self, serie_id: str | list, last_data: bool = False
    ) -> pd.DataFrame:
        """
