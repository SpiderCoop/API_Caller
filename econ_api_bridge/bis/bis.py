...
class BIS_API(BaseAPI):
    ...
    def _set_series_params(
        ...
        endpoint = (
            f"/data/{context}/{agencyID}/{resourceID}/{version}/{','.join(serie_id)}"
        )
        ...
        if last_data:
            ...
            additional_params.append("lastNObservations=1")
    ...
    def get_series_data(
        ...
        endpoint_datos, headers = self._set_series_params(
            serie_id, last_data=last_data, start_date=start_date, end_date=end_date
        )
        ...
        time_periods = data_json["data"]["structure"]["dimensions"]["observation"][0][
            "values"
        ]
        ...
        for i, (_series_key, series_content) in enumerate(series_dict.items()):
            ...
            serie = pd.Series(
                dtype=float,
                name=serie_id[i] if isinstance(serie_id, list) else serie_id,
            )
