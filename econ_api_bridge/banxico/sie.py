...
class Banxico_SIE(BaseAPI):
    ...
    def _set_series_params(
        ...
        get_series_metadata: bool = False,
    ) -> tuple:

        # Encabezados para la solicitud con el token de la API
        headers = {"Bmx-Token": self._BaseAPI__api_key}
        ...
        if isinstance(serie_id, str):
            ...
            raise ValueError(
                "El 'serie_id' debe ser una cadena de texto o una lista de cadenas de texto."
            )

        if not isinstance(last_data, bool):
            raise ValueError("last_data debe ser un valor booleano.")

        if percentage_change is not None:
            if not isinstance(percentage_change, str):
                raise ValueError("percentage_change debe ser una cadena de texto.")
            elif percentage_change not in ["PorcObsAnt", "PorcAnual", "PorcAcumAnual"]:
                raise ValueError(
                    "percentage_change debe ser uno de los siguientes valores: 'PorcObsAnt', 'PorcAnual', 'PorcAcumAnual'"
                )

        if not isinstance(no_decimals, bool):
            raise ValueError("no_decimals debe ser un valor booleano.")

        if not isinstance(get_series_metadata, bool):
            raise ValueError("get_series_metadata debe ser un valor booleano.")
        ...
        if last_data:
            ...
            if start_date is not None or end_date != pd.Timestamp.today().strftime(
                "%Y-%m-%d"
            ):
                raise ValueError(
                    "Si last_data es True, no se pueden proporcionar fechas de inicio y fin."
                )

            # Definir la URL de la API con el ID de la serie para obtener los datos de la última observación
            endpoint += "/datos/oportuno"
            ...
            try:
                ...
                raise ValueError(
                    "The provided dates must be in format 'YYYY-MM-DD'."
                ) from e
        ...
        additional_params = []
        if no_decimals:
            additional_params.append("decimales=sinCeros")
    ...
    def get_series_metadata(self, serie_id: str | list) -> dict:
        ...
        for serie_metadata in metadata_json["bmx"]["series"]:
            ...
            series_dict[serie_id] = {
                "titulo": serie_metadata["titulo"],
                "periodicidad": serie_metadata["periodicidad"],
                "cifra": serie_metadata["cifra"],
                "unidad": serie_metadata["unidad"],
            }
    ...
    def get_series_data(
        ...
        endpoint_datos, headers = self._set_series_params(
            serie_id, last_data, start_date, end_date, percentage_change, no_decimals
        )
        ...
        for serie_data in data_json["bmx"]["series"]:
            ...
            time_periods_formatted = [
                pd.to_datetime(period, dayfirst=True) for period in time_periods
            ]
